    ...
    <div class="divide-y divide-gray-100">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          ></span>
          <div>
            <span class="text-sm font-medium text-gray-700">{{ arrival.destination }}</span>
            <span v-if="arrival.route" class="text-xs text-gray-400 ml-2">{{ arrival.route }}</span>
          </div>
        </div>
        <div class="text-right">
          ...
          <span v-else-if="arrival.isDelayed" class="text-sm font-medium text-red-500">
            Delayed
          </span>
          <span v-else class="text-sm font-medium text-gray-800">
            {{ arrival.minutes }} min
          </span>
          <span v-if="arrival.isScheduled" class="text-xs text-gray-400 ml-1" title="Scheduled (not live tracked)">
            📅
          </span>
        </div>
      </div>
      ...
      <!-- Empty state -->
      <div v-if="!formattedArrivals.length" class="px-4 py-6 text-center text-gray-400 text-sm">
