		...
		>
			...
			<form @submit.prevent="handleSubmit" class="p-5 space-y-4">
				...
				<div v-if="stopType === 'train'">
					<label class="block text-sm font-medium text-gray-700 mb-2"
						>Quick Select Station</label
					>
					<select
						@change="
							(e) => {
								const s = commonStations.find(
									(st) => st.mapid === e.target.value,
								);
				...
				<div v-if="stopType === 'bus'">
					<label class="block text-sm font-medium text-gray-700 mb-1">
						Stop ID
						<span class="text-gray-400 font-normal"
							>(from bus stop sign or CTA website)</span
						>
					</label>
					<input
						v-model="stpid"
				...
				<div>
					<label class="block text-sm font-medium text-gray-700 mb-1">
						Route Filter
						<span class="text-gray-400 font-normal"
							>(optional - e.g. "Red", "Blue", or bus route "77")</span
						>
					</label>
					<input
						v-model="rt"
