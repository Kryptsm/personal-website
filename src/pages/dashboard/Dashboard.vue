	...
	return [
		{
			id: "train-1",
			type: "train",
			label: "Fullerton (Red/Brown/Purple)",
			mapid: "41220",
			rt: "",
		},
		{
	...
	return alerts.value
		.filter((alert) => {
			...
			return serviceList.some(
				(svc) =>
					trackedRoutes.has(svc.ServiceId) ||
					trackedRoutes.has(svc.ServiceName),
			);
		})
		.slice(0, 10);
});
...
function getLineColor(routeCode) {
	const line = Object.values(CTA_LINES).find(
		(l) =>
			l.code === routeCode || l.code.toLowerCase() === routeCode?.toLowerCase(),
	);
...
<template>
	<div class="dashboard-container p-6">
		...
		<div v-if="!loading">
			<section
				v-if="savedStops.filter((s) => s.type === 'train').length"
				class="mb-8"
			>
				<h2
				...
				>
					...
					>
						<path
							fill-rule="evenodd"
							d="M5 2a2 2 0 00-2 2v10a2 2 0 002 2h1l-1 2h2l1-2h4l1 2h2l-1-2h1a2 2 0 002-2V4a2 2 0 00-2-2H5zm0 2h10v5H5V4zm2.5 8a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm5 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3z"
							clip-rule="evenodd"
						/>
					</svg>
					L Train Arrivals
			...
			<!-- Bus Predictions -->
			<section
				v-if="savedStops.filter((s) => s.type === 'bus').length"
				class="mb-8"
			>
				<h2
					class="text-lg font-semibold text-gray-700 mb-3 flex items-center gap-2"
				>
					...
					>
						<path
							d="M13 3H7a4 4 0 00-4 4v4a4 4 0 001 2.65V16a1 1 0 001 1h1a1 1 0 001-1v-1h4v1a1 1 0 001 1h1a1 1 0 001-1v-2.35A4 4 0 0017 11V7a4 4 0 00-4-4zM7 5h6a2 2 0 012 2v3H5V7a2 2 0 012-2zm-1.5 8a1.5 1.5 0 110-3 1.5 1.5 0 010 3zm9 0a1.5 1.5 0 110-3 1.5 1.5 0 010 3z"
						/>
					</svg>
					Bus Arrivals
				</h2>
