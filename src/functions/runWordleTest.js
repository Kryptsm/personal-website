 ...
 * Usage:
 *   npm run test-wordle              (tests TRACE - our best performer!)
...
if (command === "compare") {
  ...
  const startingWords = ["TRACE", "CRATE", "TRAIN", "SLATE", "ADIEU", "AROSE"];
  ...
  const startingWord =
    command && command.length === 5 ? command.toUpperCase() : "TRACE";

  if (command && command.length !== 5) {
    console.log(`Invalid word "${command}" - using TRACE instead`);
  }
