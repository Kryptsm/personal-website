...
const words = JSON.parse(
  readFileSync("src/pages/wordle/words.json", "utf8")
);
...
console.log(
  `\nBenchmarking with starting word: ${startingWord}, mode: ${mode} (${maxTestWords || "ALL"} words)\n`
);
...
const result = testAlgorithmSuccessRate(words, startingWord, {
  ...
  progressCallback: (current, total, stats) => {
    ...
    if (pct >= lastProgress + 5) {
      ...
      const successRate = current > 0 ? ((won / current) * 100).toFixed(2) : "0.00";
      console.log(
