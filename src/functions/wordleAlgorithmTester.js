...
function simulateWordleFeedback(guess, targetWord) {
	...
	// Second pass: identify close letters (yellow), consuming target positions
	for (let i = 0; i < 5; i++) {
		if (correctSpots[i] === "") {
			...
			for (let j = 0; j < 5; j++) {
				if (!targetUsed[j] && targetArray[j] === letter) {
					foundInWrongPosition = true;
					targetUsed[j] = true; // Consume this target position
					break;
				}
			}
	...
	// Compute max letter counts for letters guessed multiple times
	...
	for (const [letter, count] of Object.entries(guessLetterCounts)) {
		if (count > 1 && targetArray.includes(letter)) {
			// Letter appears multiple times in guess and at least once in target
			// Count confirmed instances (green + yellow)
			let confirmedCount = 0;
			for (let i = 0; i < 5; i++) {
				if (guessArray[i] === letter) {
					if (correctSpots[i] === letter || closeSpots[i].includes(letter)) {
						confirmedCount++;
					}
				}
			}
			if (confirmedCount < count) {
				// Some instances were gray - we know max count
				maxLetterCounts[letter] = confirmedCount;
			}
		}
	}

	return { correctSpots, closeSpots, incorrectLetters, maxLetterCounts };
}
...
function mergeGameState(currentState, newState) {
	...
	// Merge max letter counts (take the minimum - more restrictive)
	const newMaxLetterCounts = { ...(currentState.maxLetterCounts || {}) };
	if (newState.maxLetterCounts) {
		for (const [letter, count] of Object.entries(newState.maxLetterCounts)) {
			if (
				newMaxLetterCounts[letter] === undefined ||
				count < newMaxLetterCounts[letter]
			) {
				newMaxLetterCounts[letter] = count;
			}
		}
	}
	...
	return {
		...
		maxLetterCounts: newMaxLetterCounts,
	};
}
